...
export interface ProfileImageHelperProps {
  /** Current avatar URL */
  currentAvatar?: string | null;
  /** Also accepted as currentImageUrl (alias used by profile-image-selector) */
  currentImageUrl?: string | null;
  /** User display name — used for initials fallback */
  displayName?: string;
  /** Called with the new public URL after upload + profile save */
  onAvatarUpdated?: (url: string) => void;
  /** Alias for onAvatarUpdated (used by profile-image-selector) */
  onSuccess?: (url: string) => void;
  /** API endpoint to PATCH/PUT the new avatarUrl (default: /api/auth/me) */
  updateEndpoint?: string;
  /** Modal open state — when provided, the component only renders when true */
  isOpen?: boolean;
  /** Called to close the modal wrapper */
  onClose?: () => void;
  /** Avatar circle size in px (default: 80) */
  size?: number;
  className?: string;
}
...
export default function ProfileImageHelper({
  ...
  const { uploadFile, isUploading, progress } = useFileUpload(BUCKET);
  ...
  // If used as a controlled modal, hide when closed
  const isModal = isOpen !== undefined;
  if (isModal && !isOpen) return null;

  const avatarSrc = preview ?? currentAvatar ?? currentImageUrl ?? null;
      ...
      {isBusy && (
        <div className="absolute inset-0 rounded-full bg-black/50 flex items-center justify-center">
          {isUploading ? (
            ...
            <Loader2
              className="text-lilac animate-spin"
